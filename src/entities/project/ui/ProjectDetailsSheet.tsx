...
import { ArrowUpRight, Download, Github, X, ChevronLeft, ChevronRight } from "lucide-react";
...
export const ProjectDetailsSheet = ({ project, children, width="600px" }: ProjectDetailsSheetProps) => {
  const [currentSlide, setCurrentSlide] = useState(0);
  
  const images = project.content.screenshots && project.content.screenshots.length > 0
        ...
        <SideSheetFooter className="absolute bottom-0 left-0 right-0 p-6 bg-bg-paper/80 backdrop-blur-xl border-t border-border-default flex flex-col sm:flex-row gap-4 sm:justify-between items-center z-40">
           ...
           <div className="w-full sm:w-auto flex-1">
             {project.content.installCommand ? (
                ...
                <a
                  href={project.githubLink ? `${project.githubLink}/releases` : "#"} // fallback
                  target="_blank"
