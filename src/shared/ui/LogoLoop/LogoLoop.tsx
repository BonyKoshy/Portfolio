...
/** Infinite scrolling loop of logos or components. */
    ...
    const renderLogoItem = useCallback(
      (item: LogoItem, key: React.Key) => {
        ...
        const content = isNodeItem ? (
          <span
            className={cx(
              ...
              scaleOnHover &&
                "transition-transform duration-300 ease-in-out [@media(hover:hover)]:group-hover/item:scale-120"
            )}
            aria-hidden={
              !!item.href && !(item as { ariaLabel?: string }).ariaLabel
            }
          >
        ...
        const itemAriaLabel = isNodeItem
          ? ((item as { ariaLabel?: string }).ariaLabel ?? item.title)
          : ((item as { alt?: string }).alt ?? item.title);
