  ...
  const panelRef = useRef<HTMLElement>(null);
    ...
    return (
      <div className={`fixed top-0 left-0 w-full h-full pointer-events-none z-50 ${open ? "is-open" : ""}`}>
        <div ref={preLayersRef} className="absolute top-0 right-0 h-full z-5 pointer-events-none w-[clamp(280px,35vw,500px)] max-lg:w-full" aria-hidden="true">
          <div className="sm-prelayer absolute top-0 right-0 h-full w-full bg-[var(--prelayer-1)]" />
          <div className="sm-prelayer absolute top-0 right-0 h-full w-full bg-[var(--prelayer-2)]" />
        </div>
        <aside
          ref={panelRef}
          className="absolute top-0 right-0 h-screen w-[clamp(280px,35vw,500px)] max-lg:w-full bg-[var(--panel-bg)] backdrop-blur-[24px] flex flex-col pt-32 px-10 pb-8 overflow-y-auto z-10 pointer-events-auto shadow-2xl"
          aria-hidden={!open}
        >
            ...
            <ul className="list-none m-0 p-0 flex flex-col gap-2 [counter-reset:smItem] w-full" data-numbering role="list">
              {items.map((it) => (
                <li className="relative overflow-hidden w-full" key={it.label}>
                  <a
                    className="sm-panel-item relative text-[var(--text-primary)] font-bold text-[3rem] max-xl:text-[2.5rem] max-md:text-[2rem] max-sm:text-[1.75rem] cursor-pointer leading-none tracking-[-1.5px] uppercase transition-colors duration-250 inline-block no-underline pr-0 hover:text-[var(--accent)] group w-full text-left"
                    href={it.link}
                    onClick={handleClose}
                  >
                    {it.label}
                    <span className="inline-block relative text-sm font-normal text-[var(--accent)] tracking-normal pointer-events-none select-none opacity-50 transition-opacity duration-300 group-hover:opacity-100 before:content-[counter(smItem,decimal-leading-zero)] [counter-increment:smItem] align-top ml-2 mt-0.5"></span>
                  </a>
                </li>
              ))}
            </ul>
            <div className="mt-auto pt-8 flex flex-col gap-3 w-full items-start" aria-label="Social links">
              <h3 className="m-0 text-base font-medium text-[var(--accent)]">Socials</h3>
                ...
                {socialItems.map((s) => (
                  <li key={s.label}>
                    <a
                      ...
                      className="text-lg font-medium text-[var(--text-primary)] no-underline relative inline-block py-0.5 transition-colors duration-300 hover:text-[var(--accent)] group"
                    >
