...
interface GlobeWireframeProps {
  ...
  markerColor?: string;
  markerLocation?: string | [number, number];
}
...
export default function GlobeWireframe({
  ...
  useEffect(() => {
    ...
    const rotateToNextCity = () => {
      ...
      const nextCity = rotateCities[nextIndex];
      if (!nextCity) return;
      const city = nextCity.toLowerCase();
    ...
    const currentCity = rotateCities[currentCityIndex];
    if (currentCity) {
  ...
  const handleMouseMove = (event: React.MouseEvent) => {
    ...
    const [lmX = 0, lmY = 0] = lastMouse;
    const dx = currentMouse[0] - lmX;
    const dy = currentMouse[1] - lmY;
  ...
  const handleMouseUp = () => {
    setIsDragging(false);
    if (!autoRotate && rotateToLocation) {
      let coords: [number, number] | undefined;
      if (typeof rotateToLocation === "string") {
        coords = cityCoordinates[rotateToLocation.toLowerCase()];
      } else {
        coords = rotateToLocation;
      }
      if (coords) {
        animateRotationTo([-coords[1], -coords[0]]);
      }
    }
  };
  ...
  const handleMouseLeave = () => {
    setIsDragging(false);
    if (!autoRotate && rotateToLocation && isDragging) {
      let coords: [number, number] | undefined;
      if (typeof rotateToLocation === "string") {
        coords = cityCoordinates[rotateToLocation.toLowerCase()];
      } else {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const targetMarker = markerLocation || rotateToLocation;
      if (targetMarker) {
        let coords: [number, number] | undefined;
        if (typeof targetMarker === "string") {
          coords = cityCoordinates[targetMarker.toLowerCase()];
        } else {
          coords = targetMarker;
        }
        if (coords) {
          ...
          // For orthographic, we check if the point is on the front hemisphere
          // Center of rotation is [-rotation[0], -rotation[1]]
          // In D3, rotation is [lambda, phi, gamma]
          const centerLon = -rotation[0];
          const centerLat = -rotation[1];
          const dist = d3.geoDistance([lon, lat], [centerLon, centerLat]);

          // Math.PI / 2 is 90 degrees. If distance is less than 90 deg, it's on the front side.
          if (p && dist < Math.PI / 2) {
            const markerGroup = svg
              .append("g")
              .attr("class", "marker-pulsing-dot");

            // Ping animation ripple matching Footer
            const pingCircle = markerGroup
              .append("circle")
              .attr("cx", p[0])
              .attr("cy", p[1])
              .attr("r", "4")
            ...
            // Solid core dot matching Footer
            markerGroup
              .append("circle")
              .attr("cx", p[0])
              .attr("cy", p[1])
  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
      <svg
        ...
        className={
          useResponsive
            ? "w-full h-full opacity-0 transition-opacity duration-1000 overflow-visible"
            : "overflow-visible"
        }
        style={{
